  ...
  private async performOCR(lead: Lead): Promise<void> {
    ...
    try {
      ...
      await db.updateLead(lead.id, {
        ...
        // Store comprehensive image analysis results including methodology steps
        source_notes: `${lead.source_notes || ''}\n\nHUMAN-LIKE VISUAL ANALYSIS METHODOLOGY:\n\nSTEP 1 - INITIAL SCAN:\nFirst Impression: ${(imageAnalysis as any).methodologySteps?.initialScan?.firstImpression || 'N/A'}\nProminent Objects: ${(imageAnalysis as any).methodologySteps?.initialScan?.prominentObjects?.join(', ') || 'N/A'}\n\nSTEP 2 - SUBJECT IDENTIFICATION:\nPrimary Subject: ${(imageAnalysis as any).methodologySteps?.subjectIdentification?.primarySubject || 'N/A'}\nText Presence Rule: ${(imageAnalysis as any).methodologySteps?.subjectIdentification?.rule1_textPresence || 'N/A'}\nCentrality Rule: ${(imageAnalysis as any).methodologySteps?.subjectIdentification?.rule2_centrality || 'N/A'}\n\nSTEP 3 - CONCEPTUAL SEGMENTATION:\nIsolation Success: ${(imageAnalysis as any).methodologySteps?.conceptualSegmentation?.isolationSuccess || false}\nExcluded Elements: ${(imageAnalysis as any).methodologySteps?.conceptualSegmentation?.excludedElements?.join(', ') || 'none'}\n\nSTEP 4 - TEXT EXTRACTION:\nAll Visible Text: ${(imageAnalysis as any).methodologySteps?.textExtraction?.allVisibleText?.join(' | ') || 'none'}\nBusiness Name: ${(imageAnalysis as any).methodologySteps?.textExtraction?.structuredData?.businessName || 'N/A'}\nPhone: ${(imageAnalysis as any).methodologySteps?.textExtraction?.structuredData?.phoneNumber || 'N/A'}\nAddress: ${(imageAnalysis as any).methodologySteps?.textExtraction?.structuredData?.address || 'N/A'}\n\nSTEP 5 - SUBJECT DESCRIPTION:\nSubject Type: ${(imageAnalysis as any).methodologySteps?.subjectDescription?.subjectType || 'unknown'}\nData Displayed: ${(imageAnalysis as any).methodologySteps?.subjectDescription?.dataDisplayed || 'N/A'}\nPurpose: ${(imageAnalysis as any).methodologySteps?.subjectDescription?.purpose || 'N/A'}\n\nTRADITIONAL ANALYSIS:\nShape: ${imageAnalysis.mainSubject.shape}\nColors: ${imageAnalysis.mainSubject.colors.join(', ')}\nLayout: ${imageAnalysis.mainSubject.layout}\nConfidence: ${imageAnalysis.confidence}\nSimilarity to OCR: ${comparison.similarity}\n\nVISUAL DESCRIPTION:\n${imageAnalysis.visualDescription}`.trim(),
      });
